...
\section{Introduction}
    ...
    The paper is organised into three sections. The first section, drawing on the work of Brian Arthur and Jessica Flack, explores a theory of technology based on complexity thinking. The second section outlines Jason Hoelscher's Complex Adaptive Systems Aesthetics, augmented with some concepts from Alicia Juarrero. The final section explores the possibility of rethinking the split between art and technology through the work of Jacques Rancière and Martin Heidegger. 
    ...
    \section{Complex Adaptive Systems Aesthetics}
    ...
    \section{Art as Technology}

    So far we have looked at how the nature of technical objects and the nature of art objects to show that, from the perspective of complexity thinking, art objects are different from other technical objects only in that the purpose of an art object always remains indeterminate. To continue maintain the position that art objects are significantly different, non-overlapping classes is to assume two things: first that technical objects which are not artworks have determinate purpose, and, second, that a difference in the nature of purpose is the difference that separates art objects from technical objects.
    
    To create the space of possibility for art to be a technology, we need to shift up a level and consider the nature of the relationship between higher-level systems: technology as a whole which includes the various technical domains, or styles, of technology, as well as art as a whole which includes the various art domains, or styles, of art. We will see that the idea that these two systems are separate and non-overlapping is an idea that has formed over time. In terms of complexity science, we can say that it is the result of a symmetry breaking cascade. Finally we will explore the possibility or rethinking the split between art and technology by problematising the assumption that technical objects have determinate purpose\footnote{Sauvagnargues has some things to say about this.}.
